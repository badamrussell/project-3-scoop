...
const DATABASE_FILE_NAME = 'scoop-database.yaml';
...
function loadDatabase() {
  fs.readFile(DATABASE_FILE_NAME, 'UTF-8', (err, res) => {
    if (err) {
      database = {
        users: {},
        articles: {},
...
function saveDatabase() {
  fs.writeFile(DATABASE_FILE_NAME, yaml.dump(database), (err, res) => {
    if (err) { throw err; }
  });
}

function hasRequiredCommentFields(commentData) {
  return (
    (commentData.body && typeof commentData.body === 'string') &&
    (commentData.username && typeof commentData.username === 'string') &&
    (commentData.articleId && typeof commentData.articleId === 'number')
  );
}
...
function isExistingArticle(articleId) {
  return !!database.articles[articleId];
}

function isExistingComment(commentId) {
  return commentId && !!database.comments[commentId];
}
...
function createComment(url, request) {
  ...
  if (!requestComment) {
    response.status = 400;
  } else if (!hasRequiredCommentFields(requestComment)) {
    response.status = 400;
  } else if (isExistingUser(requestComment.username) && isExistingArticle(requestComment.articleId)) {
...
function updateComment(url, request) {
  const id = Number(url.split('/').filter(p => p)[1]);
  ...
  if (!requestComment || !requestComment.body) {
    response.status = 400;
  } else if (isExistingComment(id)) {
    database.comments[id] = Object.assign({}, comment, {
...
function deleteComment(url, request) {
  ...
  if (isExistingComment(id)) {
    const comment = database.comments[id];
    database.users[comment.username].articleIds = database.users[comment.username].articleIds.filter(a => a !== comment.articleId);
    database.users[comment.username].commentIds = database.users[comment.username].commentIds.filter(a => a !== id);
    database.articles[comment.articleId].commentIds = database.articles[comment.articleId].commentIds.filter(a => a !== id);
...
function upvoteComment(url, request) {
  ...
  if (isExistingComment(id) && isExistingUser(username)) {
    const comment = database.comments[id];

    comment.downvotedBy = comment.downvotedBy.filter(v => v !== username);
    if (comment.upvotedBy.findIndex(v => v === username) === -1) {
...
function downvoteComment(url, request) {
  ...
  if (isExistingComment(id) && isExistingUser(username)) {
    ...
    comment.upvotedBy = comment.upvotedBy.filter(v => v !== username);
    if (comment.downvotedBy.findIndex(v => v === username) === -1) {
